...
namespace JsonTemplates
{
    ...
    ///     {path} will data bind to a value.  If it is a number it will be a number, a bool a bool etc. 
    ///     {array(path, itemTemplateName)} will process a collection of elements and bind them to the itemTemplateName
    ///     {format(path, format)} will process path and then use String.Format("format") to format the output. Example 
    ///  If you are outputing a string you can have multiple bindings in the string. If you bind to an array, it will attempt to turn
    ///  it into a string by seperating with commas and the " and "
    ///  Example: 
    ///     items: [ "item1","item2", "item3", "item4"] 
    ///     "The list is {items}".
    ///  will output as 
    ///     The list is item1, item2, item3 and item4.
    ///  You can control the seperator char and the last seperator by using the join() function
    ///     {join(path [seperator] [lastseperator])}
    ///    Example
    ///      "{join(items [; ] [ or ]}"
    ///    will output
    ...
    public class JsonTemplate
    {
        ...
        protected JObject TransformToView(JObject source)
        {
            return TransformToView(source, this.template);
        }
        ...
        private JObject TransformToView(JObject source, JObject view)
        {
            ...
            {
                int iPos = property.Key.IndexOf("=");
                if (iPos > 0)
                {
                    var key = property.Key.Substring(0, iPos);
                    var path = property.Key.Substring(iPos + 1).Trim('{','}');

                    var arr = source.SelectToken(path);
                    JObject itemTemplate = (property.Value is JArray) ? ((JArray)property.Value).First() as JObject: property.Value as JObject;
                    JArray list = new JArray();
                    foreach (var item in arr)
                    {
                        list.Add(TransformToView((JObject)item, itemTemplate));
                    }
                    target[key] = list;
                    return target;
                }
